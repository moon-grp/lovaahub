<template>
  <div>
    <v-row class="mt-2 ml-8 titl-fnt">Airtime recharge </v-row>
        ...
        <v-card>
          <v-card-text>
            <div class="text-h4 pa-4">
              confirm sending {{ amount }} airtime to {{ phone }}
            </div>
          </v-card-text>
...
export default {
  ...
  methods: {
    ...
    async buyAirtime() {
      try {
        const res = await this.$axios.$get(`${this.$config.baseUrl}airtime?`, {
          params: {
